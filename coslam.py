...
class CoSLAM():
    def __init__(self, config):
        ...
        self.dataset = get_dataset(config)  # 数据集类对象
        self.create_bounds()                # 建图的边界
        self.create_pose_data()             # 创建存储 估计的位姿 和 数据集中的位姿 gt 到用的字典
    ...
    def create_pose_data(self):
        ...
        '''
        self.est_c2w_data = {}      # 类属性：估计得到的【位姿】
        self.est_c2w_data_rel = {}  # 类属性：估计得到的【帧间的相对变化】
        self.load_gt_pose()         # 类方法：真实【位姿】
    ...
    def load_gt_pose(self):
        ...
        加载真实位姿
        '''
        self.pose_gt = {}   # 类属性：真实【位姿】
        for i, pose in enumerate(self.dataset.poses):   # 从数据集读取【下标】和【位姿】
            self.pose_gt[i] = pose                      # 保存到类属性
    ...
    def first_frame_mapping(self, batch, n_iters=100):
        ...
        for i in range(n_iters):
            ...
            rays_o = c2w[None, :3, -1].repeat(self.config['mapping']['sample'], 1)  # 相机在世界坐标系下的坐标
    ...
    def global_BA(self, batch, cur_frame_id):
        ...
        if len(self.keyframeDatabase.frame_ids) < 2:
        ...
        else:   # 如果有两个或更多关键帧，将第一个关键帧的姿态固定，并对其他关键帧姿态进行优化
            ...
            else:   # 如果不优化当前帧，get_pose_param_optim()的传参无当前帧，poses_all里加入当前帧
        ...
        current_rays = torch.cat([batch['direction'], batch['rgb'], batch['depth'][..., None]], dim=-1)# 拼接
        current_rays = current_rays.reshape(-1, current_rays.shape[-1])

        # tum.yaml里，['mapping']['iters'] = 20 , ['mapping']['sample'] = 2048
        for i in range(self.config['mapping']['iters']):    # 迭代 iters 次
    ...
    def predict_current_pose(self, frame_id, constant_speed=True):
        ...
        '''
        # 特殊处理：第二帧或者非恒速，直接使用上一帧的相机姿态作为当前帧的预测姿态
    ...
    def tracking_pc(self, batch, frame_id):
        ...
        print('Best loss: {}, Camera loss{}'.format(F.l1_loss(best_c2w_est.to(self.device)[0,:3], 
                                                              c2w_gt[:3]).cpu().item(), 
    ...
    def tracking_render(self, batch, frame_id):
        ...
        iW = self.config['tracking']['ignore_edge_W']   # replica 为例 20
        iH = self.config['tracking']['ignore_edge_H']   # replica 为例 20
        ...
        for i in range(self.config['tracking']['iter']):    # replica 数据集中迭代10次
            pose_optimizer.zero_grad()  # 清零梯度
            c2w_est = self.matrix_from_tensor(cur_rot, cur_trans)

            # Note here we fix the sampled points for optimisation
            if indice is None:
                indice = self.select_samples(self.dataset.H-iH*2, 
                                             self.dataset.W-iW*2, 
                                             self.config['tracking']['sample'])# 采样个数 replica 1024
            ...
            ret = self.model.forward(rays_o, rays_d, target_s, target_d)
            # 根据输入的相机原点(rays_o)、光线方向(rays_d)、获取光线上的采样点，把采样点输入编码网络，
            # 得到编码值，把编码值输入预测网络，得到颜色和 sdf 值。
            # 根据采样像素 rgb 值(target_s)、采样像素深度值(target_d)，与预测的颜色和 sdf 值计算loss
            # 最终返回的是各种 loss 值
        ...
        print('Best loss: {}, Last loss{}'.format(F.l1_loss(best_c2w_est.to(self.device)[0,:3], c2w_gt[:3]).cpu().item(), 
                                                  F.l1_loss(c2w_est[0,:3], c2w_gt[:3]).cpu().item()))
    ...
    def create_optimizer(self):
        ...
        if not self.config['grid']['oneGrid']:  # 不是只有一个网格
            # 往列表中添加【颜色网格】
            trainable_parameters.append({'params': self.model.embed_fn_color.parameters(), 
                                         'eps': 1e-15, 
                                         'lr': self.config['mapping']['lr_embed_color']})
        
        self.map_optimizer = optim.Adam(trainable_parameters, betas=(0.9, 0.99))    # Adam 优化器
    ...
    def run(self):
        ...
        for i, batch in tqdm(enumerate(data_loader)):   # tqdm 是显示进度条的库
            # ********************* Visualisation 可视化rgb和深度图 *********************
            if self.config['mesh']['visualisation']:
                rgb = cv2.cvtColor(batch["rgb"].squeeze().cpu().numpy(), cv2.COLOR_BGR2RGB)
                    # squeeze()后面的.cpu()删除了之后，经过测试对CPU的压力没有改善多少
            ...
            # Tracking + Mapping
                ...
                if i%self.config['mapping']['map_every']==0:  # 每5帧建一次图  ['mapping']['map_every']=5
                    self.current_frame_mapping(batch, i)    # 没用到，直接返回了。
                    # --------------------Sec 3.3 BA -------------------- 
...
if __name__ == '__main__':
    ...
    if args.output is not None: # 有指定 output 的情况下，把 yaml 文件中的 output 替换为用户输入的路径
        cfg['data']['output'] = args.output

    print("Saving config and script...")
    save_path = os.path.join(cfg["data"]["output"], cfg['data']['exp_name'])  
        # Example   save_path: "output/TUM/fr_desk/demo"
    if not os.path.exists(save_path):# 路径不存在
        os.makedirs(save_path)# 创建路径
    shutil.copy("coslam.py", os.path.join(save_path, 'coslam.py'))# 把 coslam.py 复制一份？？为什么？？

    with open(os.path.join(save_path, 'config.json'),"w", encoding='utf-8') as f:
        # 在 save_path 路径下创建 config.json 文件
        f.write(json.dumps(cfg, indent=4)) # 写入 cfg 中的内容
